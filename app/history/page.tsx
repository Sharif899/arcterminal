  ...
  return (
    <div className="wrap" style={{ padding: '32px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, letterSpacing: '-0.02em', marginBottom: 4 }}>Agent Log</h1>
          <div style={{ fontSize: 12, color: '#9199aa', fontFamily: 'Space Mono, monospace' }}>
            {address.slice(0, 10)}…{address.slice(-8)} · {filtered.length} actions · ${totalVolume.toFixed(2)} USDC volume
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="btn btn-ghost" onClick={() => load(address)} style={{ fontSize: 11 }}>↻ REFRESH</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={s.label} className="panel" style={{ padding: '14px 16px' }}>
            <div style={{ fontSize: 9, color: '#9199aa', letterSpacing: '0.15em', marginBottom: 6, fontWeight: 600 }}>{s.label}</div>
      ...
      <div className="panel" style={{ marginBottom: 16 }}>
        ...
        {loading ? (
          ...
          <div>
            ...
            {filtered.map((tx, i) => (
              <div key={tx.id}
                style={{ display: 'grid', gridTemplateColumns: '90px 80px 120px 1fr 1fr 100px 140px', gap: 12, padding: '13px 18px', borderBottom: i < filtered.length - 1 ? '1px solid #f1f3f7' : 'none', alignItems: 'center' }}
