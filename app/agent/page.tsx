...
const AGENT_WALLET = '0xA6C3...f9E2';
const CURRENCIES = ['NGN', 'GHS', 'KES', 'EUR', 'GBP', 'ZAR'];

const TYPE_STYLES: Record<AgentLog['type'], { color: string; bg: string; icon: string }> = {
  info:     { color: 'rgba(232,240,232,0.5)', bg: 'transparent',              icon: '●' },
  decision: { color: '#00aaff',               bg: 'rgba(0,170,255,0.05)',      icon: '◆' },
  action:   { color: '#ffaa00',               bg: 'rgba(255,170,0,0.05)',      icon: '▶' },
  success:  { color: '#00ff88',               bg: 'rgba(0,255,136,0.05)',      icon: '✓' },
  warning:  { color: '#ffaa00',               bg: 'rgba(255,170,0,0.05)',      icon: '⚠' },
  error:    { color: '#ff3355',               bg: 'rgba(255,51,85,0.05)',      icon: '✕' },
};
...
export default function AgentPage() {
  ...
  const [rules, setRules] = useState<AgentRule[]>([
    { id: shortId(), currency: 'NGN', condition: 'above', targetRate: 1600, action: 'convert', amount: 100, recipientWallet: '0x1234567890abcdef1234567890abcdef12345678', active: true, triggered: false },
    { id: shortId(), currency: 'GHS', condition: 'below', targetRate: 14, action: 'alert', amount: 50, recipientWallet: '', active: true, triggered: false },
  ]);
  const [liveRates, setLiveRates] = useState<Record<string, number>>({});
  const [cycleCount, setCycleCount] = useState(0);
  const [showAddRule, setShowAddRule] = useState(false);
  const [newRule, setNewRule] = useState({ currency: 'NGN', condition: 'above' as 'above' | 'below', targetRate: '', action: 'convert' as 'convert' | 'alert', amount: '', recipientWallet: '' });
  ...
  function addLog(type: AgentLog['type'], message: string, extra?: { txHash?: string; rate?: number }) {
  ...
  async function runCycle() {
    ...
    setRules(prev => {
      ...
      for (let i = 0; i < updated.length; i++) {
        ...
        const conditionMet =
          rule.condition === 'above' ? currentRate > rule.targetRate :
          currentRate < rule.targetRate;

        addLog('decision',
          `Evaluating rule: ${rule.currency} ${rule.condition} ${rule.targetRate} → current: ${currentRate.toFixed(2)} → ${conditionMet ? 'CONDITION MET ✓' : 'not yet'}`,
          { rate: currentRate }
        );
        ...
        if (conditionMet) {
          ...
          if (rule.action === 'convert' && rule.recipientWallet) {
            addLog('action', `Condition met! Triggering auto-convert: ${rule.amount} USDC → ${rule.currency} at rate ${currentRate.toFixed(2)}`);

            // Simulate Arc SDK kit.send()
            setTimeout(async () => {
              await new Promise(r => setTimeout(r, 1500));
              const txHash = mockTxHash();
              const localAmount = (rule.amount * currentRate).toFixed(2);
              addLog('success',
                `✓ Auto-conversion complete — sent ${rule.amount} USDC (≈ ${localAmount} ${rule.currency}) to ${rule.recipientWallet.slice(0, 10)}...`,
                { txHash }
              );
              setTotalTx(t => t + 1);
            ...
            addLog('warning', `🔔 ALERT: ${rule.currency} rate hit target! Current: ${currentRate.toFixed(2)} (target: ${rule.targetRate})`);
          }
        }
      }
      return updated;
  ...
  function startAgent() {
    ...
    addLog('info', `Agent wallet: ${AGENT_WALLET}`);
  ...
  function addRule() {
    ...
    setNewRule({ currency: 'NGN', condition: 'above', targetRate: '', action: 'convert', amount: '', recipientWallet: '' });
  ...
  return (
    <>
      <style>{`
        ...
        .log-container::-webkit-scrollbar-thumb { background: rgba(0,255,136,0.2); }
        .pulse-ring {
        ...
        @media (max-width: 480px) { .agent-page { padding: 16px 16px 60px !important; } }
      `}</style>
      ...
      <div className="agent-page" style={{ padding: '24px 24px 60px', maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: 'CYCLES RUN', value: cycleCount, color: '#00aaff' },
            { label: 'TX EXECUTED', value: totalTx, color: '#00ff88' },
            ...
            <div key={s.label} className="panel" style={{ padding: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {running && s.label === 'AGENT STATUS' && <div className="pulse-ring" />}
        ...
        <div className="agent-grid">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {Object.keys(liveRates).length > 0 && (
              <div className="panel" style={{ padding: '12px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', gap: 12 }}>
              ...
              <div className="log-container">
                {logs.length === 0 ? (
                  ...
                  logs.map(log => {
                    ...
                    return (
                      <div key={log.id} style={{ padding: '3px 8px', borderRadius: 4, background: style.bg, marginBottom: 2 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {showAddRule && (
                <div style={{ padding: 16, borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(0,255,136,0.02)' }}>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <input className="input" type="number" placeholder="100" value={newRule.amount} onChange={e => setNewRule(p => ({ ...p, amount: e.target.value }))} />
                  </div>
              ...
              <div>
                {rules.map(rule => (
                  <div key={rule.id} style={{
                    ...
                    background: rule.triggered ? 'rgba(0,255,136,0.04)' : 'transparent',
                    ...
                    <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, color: 'rgba(232,240,232,0.35)', marginBottom: 6 }}>
                      {rule.action === 'convert'
                        ? `→ Auto-send ${rule.amount} USDC`
                        : `→ Alert only`
                      }
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      {rule.triggered && (
                        <button onClick={() => resetRule(rule.id)} className="btn btn-blue" style={{ fontSize: 9, padding: '3px 8px' }}>RESET</button>
                      )}
                      <button
                        onClick={() => setRules(prev => prev.filter(r => r.id !== rule.id))}
                        className="btn btn-ghost"
                        style={{ fontSize: 9, padding: '3px 8px', color: '#ff3355' }}
                      >
                        DELETE
                      </button>
                    </div>
            ...
            <div className="panel" style={{ padding: 16 }}>
              ...
              {[
                { label: 'Wallet', value: AGENT_WALLET },
