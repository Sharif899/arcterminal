...
const CURRENCIES = [
  ...
  { code: 'USD', name: 'US Dollar',          flag: '🇺🇸', color: '#00ff88' },
];
...
export default function ConvertPage() {
  ...
  const usdcAmount = direction === 'toUSDC'
    ? (numAmount / rate)
    : numAmount;

  const localAmount = direction === 'fromUSDC'
    ? (numAmount * rate)
    : numAmount;
  ...
  function handleFlip() {
    setDirection(d => d === 'toUSDC' ? 'fromUSDC' : 'toUSDC');
    setAmount('');
  }
  ...
  async function handleSend() {
    ...
    setError(null);
    try {
      // Arc SDK kit.send() call
      // In production: replace with actual Arc App Kit send
  ...
  if (step === 'success') {
    return (
      <div style={{ padding: '40px 24px', maxWidth: 560, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <div className="panel" style={{ padding: 40, textAlign: 'center' }}>
          <div style={{ fontSize: 56, marginBottom: 16 }}>✅</div>
          <div style={{ fontFamily: 'Space Mono, monospace', fontWeight: 700, fontSize: 20, color: '#00ff88', marginBottom: 8 }}>
            TRANSACTION SENT
          </div>
          <div style={{ fontSize: 14, color: 'rgba(232,240,232,0.5)', marginBottom: 24 }}>
            {usdcDisplay} USDC sent successfully
          </div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={reset} className="btn btn-ghost" style={{ flex: 1, fontSize: 12 }}>
              NEW CONVERSION
            </button>
          </div>
        </div>
      ...
      <div style={{ padding: '40px 24px', maxWidth: 560, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <div className="panel">
          ...
          <div style={{ padding: 24 }}>
            ...
            {error && (
              <div style={{ marginTop: 14, padding: '10px 14px', background: 'rgba(255,51,85,0.08)', border: '1px solid rgba(255,51,85,0.2)', borderRadius: 6, fontFamily: 'Space Mono, monospace', fontSize: 12, color: '#ff3355' }}>
                ⚠ {error}
              </div>
            )}
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button onClick={() => setStep('form')} className="btn btn-ghost" style={{ flex: 1 }} disabled={sending}>
                ← BACK
              </button>
      ...
      <div className="convert-page" style={{ padding: '24px 24px 60px', maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: 'rgba(232,240,232,0.4)' }}>
            Convert any currency to USDC and send via Arc — sub-second settlement
          </p>
        </div>
        ...
        <div className="convert-grid">
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
                {(['toUSDC', 'fromUSDC'] as Direction[]).map(d => (
                  <button
                    key={d}
                    onClick={() => { setDirection(d); setAmount(''); }}
                    className={direction === d ? 'btn btn-green' : 'btn btn-ghost'}
              ...
              <div style={{ marginBottom: 16 }}>
                <label className="label">
                  {direction === 'toUSDC' ? `AMOUNT IN ${selectedCurrency}` : 'AMOUNT IN USDC'}
                </label>
                <div style={{ display: 'flex', gap: 10 }}>
                  <input
                    className="input"
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
                <button
                  ...
                  style={{ fontSize: 18, padding: '6px 16px', borderRadius: 50 }}
                >
                  ⇅
                </button>
              </div>
              ...
              <div style={{ marginBottom: 24 }}>
                <label className="label">
                  {direction === 'toUSDC' ? 'YOU GET (USDC)' : `EQUIVALENT IN ${selectedCurrency}`}
                </label>
                <div style={{
                  ...
                  <span style={{ fontSize: 13, color: 'rgba(0,255,136,0.5)', marginLeft: 8 }}>
              ...
              {/* Rate info */}
              {!loadingRates && (
                <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 6, padding: '10px 14px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontFamily: 'Space Mono, monospace', fontSize: 11, color: 'rgba(232,240,232,0.35)' }}>EXCHANGE RATE</span>
                  <span style={{ fontFamily: 'Space Mono, monospace', fontSize: 12, fontWeight: 700, color: cur?.color }}>
                    1 USDC = {rate.toLocaleString(undefined, { maximumFractionDigits: 4 })} {selectedCurrency}
                  </span>
                </div>
              )}
          ...
          {/* Side panel — currency list */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="panel">
              <div className="panel-header">
                <div className="panel-title">QUICK RATES</div>
              </div>
              <div style={{ padding: '8px 0' }}>
                {CURRENCIES.filter(c => c.code !== 'USD').map(c => {
                  ...
                  return (
                    <button
                      key={c.code}
                      onClick={() => { setSelectedCurrency(c.code); setAmount(''); }}
                      style={{
                        width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                        padding: '10px 16px', background: selectedCurrency === c.code ? 'rgba(0,255,136,0.06)' : 'transparent',
                        border: 'none', borderLeft: selectedCurrency === c.code ? '2px solid #00ff88' : '2px solid transparent',
            ...
            <div className="panel" style={{ padding: 16 }}>
              <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, color: '#00ff88', letterSpacing: '0.15em', marginBottom: 10 }}>
                ⚡ WHY ARC?
              </div>
              {[
