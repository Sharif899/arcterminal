...
const CATEGORY_COLORS: Record<string, string> = {
  Data: '#00aaff', AI: '#aa55ff', Compliance: '#ffaa00', Compute: '#00ddff', Finance: '#00ff88',
};
                ...
                <div style={{ padding: '20px 20px 0' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                    <div>
                      ...
                      <div style={{ fontFamily: 'Space Mono, monospace', fontWeight: 700, fontSize: 15, color: '#00ff88' }}>${service.pricePerCall} USDC</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 9, color: 'rgba(232,240,232,0.3)', marginBottom: 2 }}>TOTAL CALLS</div>
                      <div style={{ fontFamily: 'Space Mono, monospace', fontWeight: 700, fontSize: 15, color: '#00aaff' }}>{service.calls.toLocaleString()}</div>
                    </div>
                  </div>
                </div>
            ...
            <div className="panel" style={{ overflow: 'hidden' }}>
              ...
              {!session ? (
                <div style={{ padding: 48, textAlign: 'center' }}>
                  ...
                  <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 13, color: 'rgba(232,240,232,0.3)', marginBottom: 12 }}>No active stream</div>
                ...
                <div style={{ padding: 20 }}>
                  <div className="nano-stream-stats">
                    {[
                      ...
                      <div key={s.label} style={{ background: 'rgba(0,255,136,0.04)', border: '1px solid rgba(0,255,136,0.1)', borderRadius: 8, padding: 14 }}>
                        ...
                        <div style={{ fontFamily: 'Space Mono, monospace', fontWeight: 700, fontSize: 13, color: '#00ff88', wordBreak: 'break-word' }}>{s.value}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ height: 280, overflowY: 'auto', background: 'rgba(0,0,0,0.3)', borderRadius: 8, padding: 12 }}>
                    {transactions.filter(t => t.service === session.serviceName).slice(0, 20).map(tx => (
                      <div key={tx.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.03)', alignItems: 'center', flexWrap: 'wrap', gap: 4 }}>
                        ...
                        <span style={{ fontFamily: 'Space Mono, monospace', fontWeight: 700, fontSize: 11, color: '#00ff88' }}>${tx.amount.toFixed(4)}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div className="panel" style={{ padding: 20 }}>
                ...
                {['AI agent pays per LLM token', 'Bot pays per price feed update', 'Agent pays per compute task', 'Service pays per data query', 'Agent-to-agent commerce'].map(u => (
                  <div key={u} style={{ fontSize: 12, color: 'rgba(232,240,232,0.4)', padding: '5px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>→ {u}</div>
                ))}
              </div>
            </div>
          </div>
                  ...
                  <thead>
                    <tr>
                      {['TIME', 'SERVICE', 'AMOUNT', 'FROM', 'TX HASH', 'STATUS'].map((h, i) => (
                        <th key={h} className={i > 2 ? 'hide-mobile' : ''} style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, color: 'rgba(232,240,232,0.3)', letterSpacing: '0.15em', padding: '10px 16px', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.06)', whiteSpace: 'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
